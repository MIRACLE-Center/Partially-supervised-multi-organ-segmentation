...
def get_case_identifiers(folder):
    case_identifiers = [i[:-4] for i in os.listdir(folder) if i.endswith(
        "npz") and (i.find("segFromPrevStage") == -1)]
    return case_identifiers


def get_case_identifiers_from_raw_folder(folder):
    case_identifiers = np.unique([i[:-12] for i in os.listdir(folder)
...
def crop_2D_image_force_fg(img, crop_size, force_class=None):
    ...
    if np.any(np.array(foreground_voxels.shape) == 0):
        ...
        selected_center_voxel = foreground_voxels[:, np.random.choice(
            foreground_voxels.shape[1])]

    selected_center_voxel = np.array(selected_center_voxel)
    for i in range(2):
        selected_center_voxel[i] = max(
            crop_size[i]//2, selected_center_voxel[i])
        selected_center_voxel[i] = min(
            img.shape[i+1] - crop_size[i]//2 - crop_size[i] % 2, selected_center_voxel[i])
...
class DataLoader2D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, transpose=None,
        ...
        if transpose is not None:
            assert isinstance(
                transpose, (list, tuple)), "Transpose must be either None or be a tuple/list representing the new axis order (3 ints)"
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if not isfile(self._data[i]['data_file'][:-4] + ".npy"):
                ...
                case_all_data = np.load(
                    self._data[i]['data_file'][:-4] + ".npz")['data']
            else:
                case_all_data = np.load(
                    self._data[i]['data_file'][:-4] + ".npy", self.memmap_mode)
            ...
            if not force_fg:
                ...
                if classes_in_slice_per_axis is not None:
                    ...
                    valid_slices = np.where(np.sum(
                        case_all_data[-1] == selected_class, axis=[i for i in range(3) if i != leading_axis]))[0]
            ...
            if np.any(self.need_to_pad) > 0:
                ...
                if np.any(new_shp - np.array(self.patch_size) < 0):
                    new_shp = np.max(
                        np.vstack((new_shp[None], np.array(self.patch_size)[None])), 0)
            else:
            ...
            if new_shp is not None:
                case_all_data_donly = pad_nd_image(
                    case_all_data[:-num_seg], new_shp, self.pad_mode, kwargs=self.pad_kwargs_data)
                case_all_data_segnonly = pad_nd_image(
                    case_all_data[-num_seg:], new_shp, 'constant', kwargs={'constant_values': -1})
            ...
            if not force_fg:
                ...
                case_all_data = crop_2D_image_force_fg(
                    case_all_data[0], tuple(self.patch_size), selected_class)[None]
...
class DataLoader3DwithTag(DataLoader3D):
    def __init__(self, data, patch_size, final_patch_size, batch_size, tags, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None):
        # init_args = ()
    ...
    def generate_train_batch(self):
        content = super().generate_train_batch()
        content['tags'] = self.tags
        return content

class DataLoader3DmergeTag(DataLoader3DwithTag):
    def __init__(self, data, patch_size, final_patch_size, batch_size, tags,mergetags, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None):
        # init_args = ()
        super().__init__(data, patch_size, final_patch_size, batch_size, has_prev_stage,
        ...
        self.mergetags = [mergetag.lower() for mergetag in mergetags] 

    def generate_train_batch(self):
        content = super().generate_train_batch()
        content['tags'] = self.tags
        if not self.tags==self.mergetags:
            for i,tag in enumerate(self.tags):
                if not tag in self.mergetags:
                    content['seg'][content['seg']==i]=0
                else:
                    j = self.mergetags.index(tag)
                    content['seg'][content['seg']==i]=j
        return content

class DataLoader2DwithTag(DataLoader2D):
    def __init__(self, data, patch_size, final_patch_size, batch_size, tags, transpose=None,
                 oversample_foreground_percent=0.0, memmap_mode="r", pseudo_3d_slices=1, pad_mode="edge",
                 pad_kwargs_data=None, pad_sides=None):
        init_args = (data, patch_size, final_patch_size, batch_size, transpose,
                                            oversample_foreground_percent, memmap_mode, 
                                            pseudo_3d_slices, pad_mode,
...
if __name__ == "__main__":
    ...
    with open(join(join(preprocessing_output_dir, t), "plans_stage1.pkl"), 'rb') as f:
        plans = pickle.load(f)
    unpack_dataset(p)
    dl = DataLoader3D(dataset, (32, 32, 32), (32, 32, 32),
                      2, oversample_foreground_percent=0.33)
    dl = DataLoader3D(dataset, np.array(plans['patch_size']).astype(int), np.array(
        plans['patch_size']).astype(int), 2, oversample_foreground_percent=0.33)
    dl2d = DataLoader2D(dataset, (64, 64), np.array(plans['patch_size']).astype(
        int)[1:], 12, oversample_foreground_percent=0.33)
