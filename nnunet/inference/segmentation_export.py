...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
                                         ...
                                         interpolation_order_z: int = 0, verbose: bool = True, task:str='100'):
    """
    ...
    # if '104' in task:
    #     segmentation_softmax = np.transpose(segmentation_softmax,(0,2,3,1)) # Kitney
    # else:
    #     pass 
    # segmentation_softmax = np.transpose(segmentation_softmax,(0,2,3,1))
    current_shape = segmentation_softmax.shape
    # print("final_segmentation_predictshape___________",current_shape)
    ...
    bbox = properties_dict.get('crop_bbox') #bbox ?
...
def save_image_nifti(image, out_fname,dct,order=1,force_separate_z=None):
    """
    ...
    if isinstance(image, str):
        assert isfile(image), "If isinstance(segmentation_softmax, str) then " \
                                     "isfile(segmentation_softmax) must be True"
        del_file = deepcopy(image)
        image = np.load(image)
        os.remove(del_file)
    ...
    if np.any(np.array(current_shape) != np.array(shape_original_after_cropping)):
        if order == 0:
            seg_old_spacing = resize_segmentation(image, shape_original_after_cropping, 0, 0)
        else:
            if force_separate_z is None:
                if get_do_separate_z(dct.get('original_spacing')):
                    do_separate_z = True
