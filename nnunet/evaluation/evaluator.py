...
class Evaluator:
    ...
    def evaluate_withtags(self, test=None, reference=None,test_tags=None,reference_tags=None, advanced=False, **metric_kwargs):
        """Compute metrics for segmentations."""
        if test is not None:
            self.set_test(test)
        ...
        for i, l in enumerate(self.labels):
            if i>0 and (test_tags[l-1] in reference_tags):
                l_reference = reference_tags.index(test_tags[l-1])+1
            else:
                continue
            k = str(l)
            self.result[k] = OrderedDict()
            self.confusion_matrix.set_test(self.test == l)
            self.confusion_matrix.set_reference(self.reference == l_reference)
            for metric in eval_metrics:
                self.result[k][metric] = _funcs[metric](confusion_matrix=self.confusion_matrix,
                                                        nan_for_nonexisting=self.nan_for_nonexisting,
                                                        **metric_kwargs)
...
def run_evaluation_withtags(args):
    test, ref, evaluator,test_tags,reference_tags, metric_kwargs = args
    # evaluate
    evaluator.set_test(test)
    evaluator.set_reference(ref)
    if evaluator.labels is None:
        evaluator.construct_labels()
    current_scores = evaluator.evaluate_withtags(test_tags=test_tags,reference_tags=reference_tags,advanced=True,**metric_kwargs)
    if type(test) == str:
        current_scores["test"] = test
    if type(ref) == str:
        current_scores["reference"] = ref
...
def aggregate_scores(test_ref_pairs,
    ...
    all_res = []
    # for args in zip(test, ref, [evaluator]*len(ref), [metric_kwargs]*len(ref)):
    #     all_res.append(run_evaluation(args))
    p = Pool(num_threads)
...
def aggregate_scores_withtags(test_ref_pairs,
                     evaluator=NiftiEvaluator,
                     labels=None,
                     x_tags=None,
                     y_tags=None,
    ...
    # all_res = []
    # for args in zip(test, ref, [evaluator]*len(ref),[x_tags]*len(ref),[y_tags]*len(ref), [metric_kwargs]*len(ref)):
    #     all_res.append(run_evaluation_withtags(args))
    p = Pool(num_threads)
    all_res = p.map(run_evaluation_withtags, zip(test, ref, [evaluator]*len(ref),[x_tags]*len(ref),[y_tags]*len(ref), [metric_kwargs]*len(ref)))
    p.close()
    p.join()
    ...
    # save to file if desired
    # we create a hopefully unique id by hashing the entire output dictionary
    for l in all_scores["mean"].keys():
        dice = all_scores["mean"][l]["Dice"]
        # h = all_scores["mean"][l]["Hausdorff Distance"]
        h95 = all_scores["mean"][l]["Hausdorff Distance 95"]
        print(f"{l}:Dice / {dice}")
        # print(f"{l}:Hausdorff Distance / {h}")
        print(f"{l}:Hausdorff Distance 95 / {h95}")
    if json_output_file is not None:
        json_dict = OrderedDict()
        json_dict["name"] = json_name
        json_dict["description"] = json_description
